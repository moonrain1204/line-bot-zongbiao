...
def create_table_image_pil(df):
    # --- 終極修正：地址與描述欄位大幅加寬，畫布總寬超過 2000 像素 ---
    ...
    canvas_width = total_table_width + (padding * 2) + 100 # 給予極大側邊空間
    ...
    for r_idx, (text_list, m_lines) in enumerate(rows_data):
        ...
        for c_idx, text in enumerate(text_list):
            ...
            draw.text((curr_x + 15, y + 15), text, fill=tc, font=font if r_idx > 0 else h_font, spacing=10)
...
# ... 其餘 callback 與 handle_message 邏輯保持不變 ...
# (請參考前一次提供的完整程式碼，僅需替換 create_table_image_pil 函數即可)
