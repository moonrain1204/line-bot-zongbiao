...
def create_table_image_pil(df):
    ...
    try:
        if os.path.exists(FONT_PATH):
            ...
            # 如果找不到 myfont.ttf，這行會報錯進入 except
...
def handle_message(event):
    ...
    if msg == "總表":
        try:
            ...
            # --- 強化過濾邏輯：只保留 A 欄有明確數字或文字的列 ---
            # 1. 刪除整行都是 NaN 的列
            df = df.dropna(how='all')
            # 2. 針對第一欄(A欄)進行嚴格過濾
            df = df[df.iloc[:, 0].notna()]
            df['filter_tmp'] = df.iloc[:, 0].astype(str).str.strip().lower()
            # 排除：空白、"nan"、"none"、"." 等無效內容
            df = df[~df['filter_tmp'].isin(["", "nan", "none", ".", "0", "0.0"])]
            df = df.drop(columns=['filter_tmp'])

            if df.empty:
                line_bot_api.reply_message(event.reply_token, TextSendMessage(text="目前沒有報修資料。"))
                return
            ...
            if img_url.startswith("http"):
                ...
                line_bot_api.reply_message(event.reply_token, TextSendMessage(text="產圖成功，但上傳至圖床失敗。"))
            
            if os.path.exists(img_path): os.remove(img_path)
        except Exception as e:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text=f"讀取資料失敗，請檢查網路發佈連結。"))
    else:
